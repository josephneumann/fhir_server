...
from unkani_app.extensions import db, login_manager, ma
from unkani_app.flask_sendgrid import send_email
from unkani_app.models.fhir.phone_number import PhoneNumberAPI
from unkani_app.models.fhir.email_address import EmailAddress, EmailAddressAPI
from unkani_app.models.fhir.address import AddressAPI
from unkani_app.models.role import Role
from unkani_app.models.fhir.address import Address, AddressSchema
from unkani_app.models.fhir.phone_number import PhoneNumber
from unkani_app.models.extensions import BaseExtension
from unkani_app.models.app_group import user_app_group, AppGroup, AppGroupSchema
from unkani_app.security import app_permission_useractivation, app_permission_userforceconfirmation, \
    app_permission_userpasswordchange, app_permission_userrolechange, app_permission_userappgroupupdate
from unkani_app.utils.demographics import *
from unkani_app.utils.general import json_serial
...
class User(UserMixin, db.Model):
    ...
    def is_accessible(self, requesting_user, other_permissions=[], self_permissions=[]):
        ...
        # Stuff to check is user is accessing another user
            ...
            # Check for overlap between unkani_app groups.  If no shared group, deny access
...
class UserAPI:
    ...
    def validate_app_groups(self):
        ...
        if ids:
            ...
            if bad_ids:
                for id in bad_ids:
                    ids.discard(id)
                self.errors['warning']['bad unkani_app group ids'] = 'Some unkani_app group ids were supplied that could not match ' \
                                                              'any unkani_app groups in the system.  They were ignored.  {}'.format(
                    bad_ids)
            if valid_app_groups:
        ...
        if not self.app_groups and not self.user:
            ...
            if not self.app_groups:
                ...
                self.errors['warning']['unkani_app group missing'] = 'No valid unkani_app group id was supplied.  ' \
    ...
    def permission_check_active(self):
        ...
        :return:
            If self.active is set to a boolean value, the requesting authenticated user must have the unkani_app-permission
    ...
    def permission_check_confirmed(self):
        ...
        :return:
            If self.confirmed is set to a boolean value, the requesting authenticated user must have the unkani_app-permission
    ...
    def permission_check_password(self):
        ...
        :return:
            If a user object already exists (is being updated) and self.password passes validation, the user must
                have the unkani_app-permission 'user password change'.  If permission check fails, self.password is set
    ...
    def permission_check_role_id(self):
        ...
        :return:
            If self.role id is supplied and passes validation, the user must have the unkani_app-permission
    ...
    def permission_check_app_groups(self):
        if self.app_groups:
            if not app_permission_userappgroupupdate.can():
                if self.user:
                    self.app_groups = None
                    self.errors['warning']['unkani_app group permission'] = 'The authenticated user does not have access' \
                                                                     ' to assign unkani_app groups.  The existing users ' \
                                                                     'unkani_app group was not updated.'
                else:
                    self.app_groups = [AppGroup.query.filter(AppGroup.default == True).first()]
                    self.errors['warning']['unkani_app group permission'] = 'The authenticated user does not have access' \
                                                                     ' to assign unkani_app groups.  The default unkani_app group' \
                ...
                for ag in self.app_groups:
                    ...
                    if not self.app_groups:
                        if self.user:
                            self.app_groups = None
                            self.errors['warning'][
                                'unkani_app group membership'] = 'Authenticated user attempted to assign the' \
                                                          ' existing user to one or more unkani_app groups ' \
                                                          'of which the authenicated user is not a member.  The ' \
                                                          'existing unkani_app group was retained.  Bad unkani_app groups: {}'.format(
                                restricted_ags)
                        else:
                            self.errors['critical'][
                                'unkani_app group membership'] = 'Authenticated user attempted to assign the' \
                                                          ' new user to an unkani_app group that they do not' \
    ...
    def make_object(self):
        ...
        # Do the dirty work of creating the user now
            ...
            if isinstance(self.address, Address):
                ...
                if self.app_groups:  # Overwrite unkani_app groups if new ones are supplied
