...
from app.main.utils.operation_outcome import create_operation_outcome
...
def authentication_error_handler(e):
    response = fhir_error_response(status_code=401, outcome_list=[
        {'severity': 'error', 'type': 'security', 'location': [url_for('auth.new_token', _external=True)],
         'diagnostics': e.args[0], 'details': 'Basic authentication error: {}'.format(e.args[0])}])
    response.headers['Location'] = url_for('auth.new_token', _external=True)
    response.headers['WWW-Authenticate'] = 'Basic'
    return response


@bp_errors.app_errorhandler(BasicAuthError)
def basic_auth_error_handler(e):
    response = fhir_error_response(status_code=401, outcome_list=[
        {'severity': 'error', 'type': 'login', 'location': [url_for('auth.new_token', _external=True)],
         'diagnostics': e.args[0], 'details': 'Basic authentication error: {}'.format(e.args[0])}])
    response.headers['Location'] = url_for('auth.new_token', _external=True)
    response.headers['WWW-Authenticate'] = 'Basic'
    return response
...
@bp_errors.app_errorhandler(TokenAuthError)
...
@bp_errors.app_errorhandler(TokenExpiredError)
def token_expired_error_handler(e):
    response = fhir_error_response(status_code=401, outcome_list=[
        {'severity': 'error', 'type': 'expired', 'location': [url_for('auth.new_token', _external=True)],
...
@bp_errors.app_errorhandler(RateLimitError)
...
@bp_errors.app_errorhandler(BadRequestError)
...
@bp_errors.app_errorhandler(ForbiddenError)
...
@bp_errors.app_errorhandler(NotModifiedError)
...
@bp_errors.app_errorhandler(PreconditionFailedError)
def not_modified_handler(e):
