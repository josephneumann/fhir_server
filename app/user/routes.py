...
from app.user.models.user import User, UserAPI, UserVersionSchema
...
from app.main.utils.pagination import paginate_query, filter_ops, register_arg_error
...
@bp_user.route('/User/<int:userid>', methods=['GET'])
@token_auth.login_required
@rate_limit(limit=5, period=15)
@etag
def get_user(userid):
    ...
    response.headers['Location'] = url_for('user.get_user', userid=user.id)
...
@bp_user.route('/User/<int:userid>', methods=['PATCH'])
@token_auth.login_required
@rate_limit(limit=5, period=15)
...
@bp_user.route('/User/<int:userid>', methods=['DELETE'])
@token_auth.login_required
@rate_limit(limit=5, period=15)
...
@bp_user.route('/User/<int:userid>/version/<int:version_number>', methods=['GET'])
@token_auth.login_required
@rate_limit(limit=5, period=15)
def get_user_version(userid, version_number):
    ...
    self_url = url_for('user.get_user_version', userid=user.id, version_number=version_number, _external=True)
    ...
    first_url = url_for('user.get_user_version', userid=user.id, version_number=1, _external=True)
    last_url = url_for('user.get_user_version', userid=user.id, version_number=len(user.versions.all()),
                       _external=True)

    if uv.next:
        next_url = url_for('user.get_user_version', userid=user.id, version_number=version_number + 1, _external=True)
    else:
        next_url = None

    if uv.previous:
        previous_url = url_for('user.get_user_version', userid=user.id, version_number=version_number - 1,
                               _external=True)
    else:
        previous_url = None
