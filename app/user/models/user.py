...
from app.user.models.app_group import user_app_group, AppGroup, AppGroupSchema
from app.fhir.models.phone_number import PhoneNumberAPI
from app.fhir.models.email_address import EmailAddress, EmailAddressAPI
from app.fhir.models.address import AddressAPI
from app.fhir.models.address import Address, AddressSchema
from app.fhir.models.phone_number import PhoneNumber
from app.auth.security import app_permission_useractivation, app_permission_userforceconfirmation, \
...
class User(db.Model):
    ...
    def get_url(self):
        ...
        """
        return url_for('user.get_user', userid=self.id, _external=True)
    ...
    def previous_version_url(self):
        ...
        if self.versions and self.version_number > 1:
            return url_for('user.get_user_version', userid=self.id, version_number=self.version_number - 1,
                           _external=True)
        else:
            return None
...
class UserVersionSchema(ma.Schema):
    ...
    def get_transaction_data(self, user_version):
        ...
        if tx_userid:
            tx_user_url = url_for('user.get_user', userid=tx_userid, _external=True)
        else:
            tx_user_url = None
    ...
    def get_user_url(self, user_version):
        ...
        return url_for('user.get_user', userid=user_version.id, _external=True)
